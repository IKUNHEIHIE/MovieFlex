...
export default function FurinaMascot() {
  ...
  const [chatMessages, setChatMessages] = useState<ChatMessage[]>([
    { role: 'assistant', content: '你好，我是芙宁娜。想找电影，还是想了解 MovieFlex？' },
  ]);
  ...
  const suppressed = !chatOpen && isMascotSuppressed(pathname, focusedEditable);
  ...
  const sendChatMessage = async () => {
    ...
    setChatInput('');
    setChatMessages((messages) => [...messages, { role: 'user', content: message }, { role: 'assistant', content: '' }]);
    setChatLoading(true);
    try {
      ...
      setChatMessages((messages) => [...messages.slice(0, -1), { role: 'assistant', content: '网络连接出现问题，请稍后再试。' }]);
    } finally {
  ...
  const activateMascot = (event: React.MouseEvent<HTMLButtonElement>) => {
    if (shouldActivateMascotClick(event.detail)) openChat();
  };
  ...
  const release = () => {
    ...
    if (!wasDragging) openChat();
  };

  return (
    <aside className={styles.mascot} aria-label="芙宁娜看板娘" style={{ transform: `translate3d(${position.x}px, ${position.y}px, 0)` }}>
      {dialogue && <div className={`${styles.speech} ${position.x < 230 ? styles.speechRight : ''}`} role="status">{dialogue}</div>}
      {chatOpen && (
        <section className={`${styles.chatPanel} ${position.x < 430 ? styles.chatPanelRight : ''}`} aria-label="芙宁娜智能助手">
          <header className={styles.chatHeader}>
            <div><strong>芙宁娜助手</strong><span>MovieFlex 智能问答</span></div>
            <button type="button" className={styles.chatClose} aria-label="关闭聊天" onClick={() => setChatOpen(false)}>x</button>
          </header>
          <div className={styles.chatMessages} aria-live="polite">
            {chatMessages.map((item, index) => (
              <p key={`${item.role}-${index}`} className={item.role === 'user' ? styles.chatUserMessage : styles.chatAssistantMessage}>{item.content}</p>
            ))}
            {chatLoading && <p className={styles.chatAssistantMessage}>正在思考...</p>}
          </div>
          <form className={styles.chatForm} onSubmit={(event) => { event.preventDefault(); void sendChatMessage(); }}>
            <input value={chatInput} onChange={(event) => setChatInput(event.target.value)} placeholder="想问点什么？" maxLength={1000} disabled={chatLoading} aria-label="输入问题" />
            <button type="submit" disabled={!chatInput.trim() || chatLoading} aria-label="发送问题">发送</button>
          </form>
        </section>
      )}
      <button type="button" className={styles.character} aria-label="点击芙宁娜打开智能助手，长按后可拖动"

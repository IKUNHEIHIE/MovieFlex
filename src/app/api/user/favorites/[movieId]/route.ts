...
export async function POST(_request: NextRequest, { params }: { params: Promise<{ movieId: string }> }) {
  ...
  await prisma.userFavorite.upsert({ where: { uk_user_movie: { userId: actor.userId, movieId } }, create: { userId: actor.userId, movieId }, update: {} });
  await sendKafkaMessage('user-behaviors', {
    eventType: 'favorite',
    userId: actor.userId,
    movieId,
    data: { action: 'add' }

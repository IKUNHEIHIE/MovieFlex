...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!message) return NextResponse.json({ error: '请输入想问的问题。' }, { status: 400 });
    if (message.length > 1000) return NextResponse.json({ error: '单次提问不能超过 1000 个字符。' }, { status: 400 });

    const apiKey = process.env.SENSENOVA_API_KEY;
    const model = process.env.SENSENOVA_MODEL ?? 'sensenova-6.7-flash-lite';
    if (!apiKey) return NextResponse.json({ error: 'AI 服务尚未配置 API Key。' }, { status: 500 });
